...
exports.saveTargetStorageKey = void 0;
exports.registerSettingsUiCommand = registerSettingsUiCommand;
...
exports.saveTargetStorageKey = "faah.settings.saveTarget.v1";
const validPresetIds = new Set([
    "balanced",
    "quiet",
    "aggressive",
]);
function normalizeSaveTarget(value, hasWorkspace) {
    return value === "workspace" && hasWorkspace ? "workspace" : "global";
}
async function rememberSaveTarget(context, target) {
    if (typeof context.globalState?.update !== "function")
        return;
    try {
        await context.globalState.update(exports.saveTargetStorageKey, target);
    }
    catch (err) {
...
function renderSettingsWebview(webview, context, settings, hasWorkspace, terminalMonitoringCapability, initialSaveTarget = "global") {
    ...
    const selectedSaveTarget = normalizeSaveTarget(initialSaveTarget, hasWorkspace);
    const terminalStatusMessage = terminalMonitoringCapability === "full"
...
<body>
      ...
      <article class="card full">
        ...
        <div class="row">
          <div>
            ...
            <div class="hint">${terminalMonitoringSupported
        ? terminalMonitoringCapability === "exitCodeOnly"
        ...
        : "Unavailable in this host. Your saved preference is preserved, but runtime monitoring is disabled here."}</div>
          </div>
          <button id="monitorTerminalSwitch" class="switch" type="button" aria-label="Toggle terminal monitoring" ${terminalMonitoringSupported ? "" : "disabled"}><span></span></button>
        </div>
  ...
  <script nonce="${nonce}">
    ...
    const defaults = ${JSON.stringify(settings_1.defaultStoredSettings).replace(/</g, "\\u003c")};
    const excludePresetDefinitions = ${JSON.stringify(settings_1.excludePatternPresetDefinitions).replace(/</g, "\\u003c")};
...
function registerSettingsUiCommand(context, getStoredSettings, onSaved, onTest, terminalMonitoringCapability, commandId = commands_1.commandIds.openSettingsUi) {
    let panel;
    function getInitialSaveTarget() {
        return normalizeSaveTarget(typeof context.globalState?.get === "function"
            ? context.globalState.get(exports.saveTargetStorageKey)
            : undefined, (vscode.workspace.workspaceFolders?.length ?? 0) > 0);
    }
    function hasWorkspaceOpen() {
        return (vscode.workspace.workspaceFolders?.length ?? 0) > 0;
    }
    function getPersistTargetFromMessage(messageTarget) {
        return messageTarget === "workspace" && hasWorkspaceOpen()
            ? "workspace"
            : "global";
    }
    function renderPanelHtml() {
        return renderSettingsWebview(panel.webview, context, getStoredSettings(), hasWorkspaceOpen(), terminalMonitoringCapability, getInitialSaveTarget());
    }
    const commandDisposable = vscode.commands.registerCommand(commandId, async () => {
        ...
        panel = vscode.window.createWebviewPanel("faahSettings", "Faah Settings", vscode.ViewColumn.One, {
            enableScripts: true,
        ...
        panel.webview.onDidReceiveMessage(async (message) => {
            ...
            if (message.type === "test") {
                const normalized = (0, settings_1.normalizeStoredSettings)((message.payload ?? {}));
                onTest(normalized);
                return;
            }
            ...
            if (message.type === "applyPreset") {
                try {
                    ...
                    const presetSettings = (0, settings_1.createPresetSettings)(getStoredSettings(), presetId, terminalMonitoringCapability !== "none");
                    ...
                    panel?.webview.postMessage({
                        ...
                        skippedConfigurationKeys: savedResult && typeof savedResult === "object"
            ...
            if (message.type === "exportSettings") {
                try {
                    ...
                    if (!targetUri)
                        return;
                    await fs.writeFile(targetUri.fsPath, `${JSON.stringify(normalized, null, 2)}\n`, "utf8");
                    panel?.webview.postMessage({
                        type: "exported",
            ...
            if (message.type === "importSettings") {
                try {
                    ...
                    const normalized = (0, settings_1.normalizeStoredSettings)(parsed);
                    const persistTarget = getPersistTargetFromMessage(message.target);
    ...
    return vscode.Disposable.from(commandDisposable, configurationDisposable, ...(workspaceFoldersDisposable ? [workspaceFoldersDisposable] : []));
}
//# sourceMappingURL=settings-webview.js.map